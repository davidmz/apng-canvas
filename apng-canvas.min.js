...
a.setAttribute("type","text/vbscript");a.text="Function APNGIEBinaryToBinStr(Binary)\r\n   APNGIEBinaryToBinStr = CStr(Binary)\r\nEnd Function\r\n";document.body.appendChild(a)});var s=[],v={},q=[];(function(){if(q.length)for(var a=q.splice(0,q.length),b=(new Date).getTime();a.length;)a.shift().call(null,b);setTimeout(arguments.callee,1E3/60)})();var x=b.requestAnimationFrame||b.webkitRequestAnimationFrame||b.mozRequestAnimationFrame||b.oRequestAnimationFrame||function(a){q.push(a)},z=function(a){for(var b=
0;b<s.length;b++)for(var c=s[b];c.isActive&&c.nextRenderTime<=a;)c.renderFrame(a);x(z)};x(z);var B=function(){this.isActive=!1;this.numPlays=this.height=this.width=this.nextRenderTime=0;this.frames=[];this.playTime=0;var b=new Deferred;this.whenReady=h(b.promise,b);this.parsePNGData=function(c){if(c.substr(0,8)!=o)return b.reject("Invalid PNG file signature"),b.promise();var e,A="",i="",f=!1,j=8,g=null;do{var h=a(c.substr(j,4)),k=c.substr(j+4,4),m;switch(k){case "IHDR":e=m=c.substr(j+8,h);this.width=
a(m.substr(0,4));this.height=a(m.substr(4,4));break;case "acTL":f=!0;this.numPlays=a(c.substr(j+8+4,4));break;case "fcTL":g&&this.frames.push(g);m=c.substr(j+8,h);g={};g.width=a(m.substr(4,4));g.height=a(m.substr(8,4));g.left=a(m.substr(12,4));g.top=a(m.substr(16,4));var n=l(m.substr(20,2)),w=l(m.substr(22,2));w==0&&(w=100);g.delay=1E3*n/w;this.playTime+=g.delay;g.disposeOp=m.charCodeAt(24);g.blendOp=m.charCodeAt(25);g.dataParts=[];break;case "fdAT":g&&g.dataParts.push(c.substr(j+8+4,h-4));break;
case "IDAT":g&&g.dataParts.push(c.substr(j+8,h));break;case "IEND":i=c.substr(j,h+12);break;default:A+=c.substr(j,h+12)}j+=12+h}while(k!="IEND"&&j<c.length);g&&this.frames.push(g);if(!f)return b.reject("Non-animated PNG"),b.promise();for(var q=0,s=this,c=0;c<this.frames.length;c++){f=new Image;g=this.frames[c];g.img=f;f.onload=function(){q++;q==s.frames.length&&b.resolve(this)};f.onerror=function(){b.reject("Image creation error")};j=new r;j.append(o);e=p(g.width)+p(g.height)+e.substr(8);j.append(y("IHDR",
e));j.append(A);for(h=0;h<g.dataParts.length;h++)j.append(y("IDAT",g.dataParts[h]));j.append(i);f.src=j.getUrl("image/png");delete g.dataParts}return b.promise()};var e=[];this.addCanvas=function(){var a=document.createElement("canvas");a.width=this.width;a.height=this.height;var c=a.getContext("2d");e.push(c);e.length>1&&x(function(){c.putImageData(e[0].getImageData(0,0,a.width,a.height),0,0)});this.isActive=!0;return a};var c=0,i=null,f=function(a,c){for(var b=0;b<e.length;b++)e[b][a].apply(e[b],
c)};this.renderFrame=function(a){if(e.length!=0){var b=c++%this.frames.length,d=this.frames[b];if(b==0&&(f("clearRect",[0,0,this.width,this.height]),i=null,d.disposeOp==2))d.disposeOp=1;i&&i.disposeOp==1?f("clearRect",[i.left,i.top,i.width,i.height]):i&&i.disposeOp==2&&f("putImageData",[i.iData,i.left,i.top]);i=d;i.iData=null;if(i.disposeOp==2)i.iData=e[0].getImageData(d.left,d.top,d.width,d.height);d.blendOp==0&&f("clearRect",[d.left,d.top,d.width,d.height]);f("drawImage",[d.img,d.left,d.top]);if(this.numPlays==
0||c/this.frames.length<this.numPlays){if(this.nextRenderTime==0)this.nextRenderTime=a;for(;a>this.nextRenderTime+this.playTime;)this.nextRenderTime+=this.playTime;this.nextRenderTime+=d.delay}else this.isActive=!1}}},C=function(a){var e=new Deferred,c=new XMLHttpRequest,f=b.BlobBuilder||b.WebKitBlobBuilder,h=typeof c.responseBody!="undefined",k=typeof c.responseType!="undefined"&&typeof f!="undefined",l=typeof c.overrideMimeType!="undefined"&&!k;c.open("GET",a,!0);k?c.responseType="arraybuffer":
l&&c.overrideMimeType("text/plain; charset=x-user-defined");c.onreadystatechange=function(){if(this.readyState==4&&this.status==200)if(k){var a=new f;a.append(this.response);var b=new FileReader;b.onload=function(){e.resolve(this.result)};b.readAsBinaryString(a.getBlob())}else{a="";if(h)for(var b=APNGIEBinaryToBinStr(this.responseBody),d=0,j=b.length;d<j;d++){var g=b.charCodeAt(d);a+=String.fromCharCode(g&255,g>>8&255)}else{b=this.responseText;d=0;for(j=b.length;d<j;++d)a+=String.fromCharCode(b.charCodeAt(d)&
255)}e.resolve(a)}else this.readyState==4&&e.reject(c)};c.send();return e.promise()}})();
